...
class PatientRetriever:
    ...
    def _safe_read_csv(proj_directory: str, fname: str, nrows):
        try:
            ...
            return pd.read_csv(filepath_or_buffer=filepath, usecols=['patient_uid', 'patient', 'similar_patients'],
                               nrows=nrows)
    ...
    def metrics_to_json(self):
        ...
        self.dataset['similar_patients'] = self.dataset['similar_patients'].apply(
            lambda x: PatientRetriever._string_to_list(x))
        self.dataset['doc2vec_precision'], self.dataset['doc2vec_recall'] = zip(
            *self.dataset.apply(
                lambda row: PatientRetriever._calc_precision_recall(row['similar_patients'], row[self.column_names[1]]),
                axis=1))
        self.dataset['tfidf_precision'], self.dataset['tfidf_recall'] = zip(
            *self.dataset.apply(
                lambda row: PatientRetriever._calc_precision_recall(row['similar_patients'], row[self.column_names[0]]),
                axis=1))
        metrics = self.dataset[
            ['patient_uid', 'doc2vec_precision', 'doc2vec_recall', 'tfidf_precision', 'tfidf_recall']]
    ...
    def calc_overlap(self):
        ...
        self.dataset['overlap'] = self.dataset.apply(
            lambda row: len(set(row[self.column_names[0]]) & set(row[self.column_names[1]])), axis=1)
        return self.dataset['overlap'].mean()
